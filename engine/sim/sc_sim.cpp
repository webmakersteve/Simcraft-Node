...
bool progress_bar_t::update( bool finished )
{
  ...
  int current, _final;
  double pct = sim.progress( &current, &_final ); //@todostephen maybe link the hook in here 
...
bool sim_t::iterate()
{
  ...
  for( int i = 0; use_lb ? (true) : (i < iterations); ++i )
  {
    do_pause();
      ...
      // This is really where i want to be. Iterate is called by execute.
                        ...
                        std::string* detailed )
{
  ...
  if ( _final   ) *_final   = total_iterations;
    ...
    // @ Use a compiler option to ignore this function below.
